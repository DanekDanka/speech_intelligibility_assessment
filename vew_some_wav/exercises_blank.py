...
# GMM VAD implementation from sr_labs_book/lab2
...
def gauss_pdf(value, m, sigma):
    """Gaussian PDF (from lab2). value, m, sigma can be arrays; sigma guarded from zero."""
    sigma = np.abs(sigma) + 1e-10
    return (1.0 / (sigma * np.sqrt(2 * np.pi))) * np.exp(-(value - m) ** 2 / (2 * sigma ** 2))
