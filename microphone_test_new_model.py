...
def record_audio():
    print(f"\nГоворите в микрофон {Config.DURATION} секунд...")
    audio = sd.rec(
...
def energy_based_vad(y, sr, aggressiveness=3, frame_duration_ms=30):
    """
    Voice Activity Detection с использованием WebRTC VAD
    
    Параметры:
        y (np.array): Аудиосигнал (должен быть 16-битным моно PCM)
        sr (int): Частота дискретизации (поддерживает 8000, 16000, 32000, 48000 Гц)
        aggressiveness (int): Агрессивность VAD (0-3, где 3 самый строгий)
        frame_duration_ms (int): Длительность фрейма в миллисекундах (10, 20 или 30)
        
    Возвращает:
        np.array: Аудио только с речевыми сегментами
    """
    # Проверка поддерживаемых параметров
    if sr not in [8000, 16000, 32000, 48000]:
        raise ValueError("WebRTC VAD поддерживает только sr=8000,16000,32000,48000")
    
    if frame_duration_ms not in [10, 20, 30]:
        raise ValueError("Поддерживаются frame_duration_ms=10,20,30")
    
    if aggressiveness not in [0, 1, 2, 3]:
        raise ValueError("aggressiveness должен быть 0,1,2,3")
    ...
    frames = np.array_split(y, np.arange(frame_length, len(y), frame_length))
    
    # Определение речевых фреймов
    speech_frames = []
    for frame in frames:
        if len(frame) < frame_length:
            continue  # Пропускаем неполные фреймы
...
def process_audio(audio):
    try:
        ...
        # VAD
        speech_only = energy_based_vad(audio_vad, Config.TARGET_SR)#, **Config.VAD_PARAMS)
...
def estimate_snr(audio, sr=16000, frame_length=2048, hop_length=512, use_nn=True):
    """
    Оценка SNR с использованием библиотеки noisereduce
    
    Параметры:
        audio (np.array): Аудиосигнал
        sr (int): Частота дискретизации
        frame_length (int): Размер окна для анализа
        hop_length (int): Шаг между окнами
        use_nn (bool): Использовать нейросетевой метод (если True)
        
    Возвращает:
        float: Оценка SNR в dB
    """
    try:
        ...
        if use_nn:
            ...
            noisy_part = audio[:frame_length*3]  # Первые 10 фреймов как шум
            ...
            noisy_part = audio[:frame_length*3]  # Первые 3 фрейма как шум
        ...
        # Расчет SNR
        snr_linear = signal_energy / (noise_energy + 1e-10)  # Добавляем маленькое значение для стабильности
        snr_db = 10 * np.log10(snr_linear)
        
        return snr_db
        return max(0.0, snr_db)  # Ограничиваем снизу нулем
...
def predict_stoi(model, audio, sr):
    ...
    n_samples = Config.TARGET_SR * Config.DURATION
    ...
    return max(0.0, min(1.0, prediction.item()))  # Ограничиваем диапазон [0, 1]
